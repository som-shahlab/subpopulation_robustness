...
from group_robustness_fairness.prediction_utils.extraction_utils.database import (
    BQDatabase,
)
...
class CountDelayedQuery(FeatureQuery):
    """
    A query that counts occurrences of concept_ids, mapping the concept date to the discharge time
    """
        ...
        return """
            WITH source_table as (
                SELECT 
                    ...
                    CAST(t2.{index_date_field} AS DATE) as index_date, 
                    {concept_id} AS concept_id, 
                    t4.visit_end_date AS concept_date,
                    '{analysis_id}' AS analysis_id,
                    CONCAT('bin_', {bin_left}, '_', {bin_right}) AS time_bin
                FROM {dataset_project}.{dataset}.{source_table} t1
                INNER JOIN {rs_dataset_project}.{rs_dataset}.{cohort_name} AS t2 ON
                ...
                INNER JOIN {dataset_project}.{dataset}.concept AS t3 ON
                    t1.{concept_id} = t3.concept_id
                INNER JOIN {dataset_project}.{dataset}.visit_occurrence AS t4 ON
                    t1.visit_occurrence_id = t4.visit_occurrence_id
                WHERE 
                    CAST(t4.visit_end_date AS DATE) BETWEEN 
                        DATE_ADD(CAST(t2.{index_date_field} AS DATE), INTERVAL {bin_left} DAY) AND
                        DATE_ADD(CAST(t2.{index_date_field} AS DATE), INTERVAL {bin_right} DAY)
                    AND standard_concept = 'S'
                {limit_str}
...
class NoteNLPCountDelayedQuery(CountQuery):
    def get_base_query(self):
        """
        A generic query that can be used to get the counts of unique concepts.
        Allows for binning by time
        ...
        return """
            WITH source_table as (
                ...
                INNER JOIN {dataset_project}.{dataset}.concept AS t4 ON
                    t3.note_nlp_concept_id = t4.concept_id
                INNER JOIN {dataset_project}.{dataset}.visit_occurrence AS t5 ON
                    t1.visit_occurrence_id = t5.visit_occurrence_id
                WHERE 
                    CAST(t5.visit_end_date AS DATE) BETWEEN 
                        DATE_ADD(CAST(t2.{index_date_field} AS DATE), INTERVAL {bin_left} DAY) AND
                        DATE_ADD(CAST(t2.{index_date_field} AS DATE), INTERVAL {bin_right} DAY)
                    AND standard_concept = 'S'
                {limit_str}
    ...
    def get_query_config(self):
        return {"analysis_id": "note_nlp_delayed"}
...
class ConditionOccurrenceCountDelayedQuery(CountDelayedQuery):
    def get_query_config(self):
        return {
            "concept_id": "condition_concept_id",
            "source_table": "condition_occurrence",
            "analysis_id": "condition_occurrence_delayed",
        }


class ProcedureOccurrenceCountDelayedQuery(CountDelayedQuery):
    def get_query_config(self):
        return {
            "concept_id": "procedure_concept_id",
            "source_table": "procedure_occurrence",
            "analysis_id": "procedure_occurrence_delayed",
        }


class NoteTypeCountDelayedQuery(CountDelayedQuery):
    def get_query_config(self):
        return {
            "concept_id": "note_type_concept_id",
            "source_table": "note",
            "analysis_id": "note_type_delayed",
        }
...
class BigQueryOMOPFeaturizer:
    ...
    def __init__(self, **kwargs):
        ...
        if self.config_dict["time_bins_hourly"] is not None:
    ...
    def get_default_queries(self):
        query_classes = [
            ...
            ConditionOccurrenceCountDelayedQuery(),
            DrugExposureCountQuery(),
            DeviceExposureCountQuery(),
            MeasurementCountQuery(),
            # ProcedureOccurrenceCountQuery(),
            ProcedureOccurrenceCountDelayedQuery(),
            # NoteTypeCountQuery(),
            # NoteNLPCountQuery(),
            NoteTypeCountDelayedQuery(),
            NoteNLPCountDelayedQuery(),
    ...
    def featurize(self, analysis_ids=None, exclude_analysis_ids=None):
        ...
        for analysis_id in analysis_ids:
            if analysis_id not in self.query_dict.keys():
                raise ValueError(
                    "Provided analysis_id {} not defined".format(analysis_id)
                )
        ...
        for analysis, query in query_dict.items():
            ...
            elif query.config_dict["requires_time_bin_hourly"] and (
                self.config_dict["time_bins_hourly"] is not None
            ):
            ...
            elif (not query.config_dict["requires_time_bin"]) and (
                not query.config_dict["requires_time_bin_hourly"]
            ):
                formatted_query = query.base_query.format_map(
