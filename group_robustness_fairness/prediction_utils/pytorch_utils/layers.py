...
class EmbeddingBagLinear(nn.Module):
    ...
    def compute_offsets(self, batch):
        return (
            torch.tensor(batch.indices, dtype=torch.long).to(
                self.embed.weight.device, non_blocking=True,
            ),
            torch.tensor(batch.indptr[:-1], dtype=torch.long).to(
                self.embed.weight.device, non_blocking=True
            )
            if batch.shape[0] > 1
            else torch.tensor([0], dtype=torch.long).to(
                self.embed.weight.device, non_blocking=True
            ),
            torch.tensor(batch.data, dtype=torch.float).to(
                self.embed.weight.device, non_blocking=True
            ),
        )
...
class EmbeddingBagLinearDict(EmbeddingBagLinear):
    def compute_offsets(self, batch):
        ...
        lengths = torch.tensor(
            [x.shape[-1] for x in batch["col_id"]], device=device, dtype=torch.long
        )
        ...
        return (
            torch.cat(batch["col_id"]).to(self.embed.weight.device, non_blocking=True),
            indptr,
            torch.cat(batch["data"]).to(self.embed.weight.device, non_blocking=True),
        )


class EmbeddingBagLinearList(EmbeddingBagLinear):
    def compute_offsets(self, batch):
        device = self.embed.weight.device
        lengths = torch.tensor(
            [x.shape[-1] for x in batch], device=device, dtype=torch.long
        )
        ...
        return (
            torch.cat(batch).to(device, non_blocking=True),
            indptr,
        )
...
class LinearLayerWrapper(torch.nn.Module):
    ...
    def __init__(
        ...
        elif sparse and (sparse_mode == "list"):
            self.linear = EmbeddingBagLinearList(in_features, out_features)
        else:
            self.linear = nn.Linear(in_features, out_features)
