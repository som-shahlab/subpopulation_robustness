...
from group_robustness_fairness.prediction_utils.util import df_dict_concat
...
if __name__ == "__main__":
    ...
    experiment_names = {
        ...
        "erm_group_aware": "{}_erm_group_aware".format(task_prefix),
        "erm_subset": "{}_erm_subset_tuning".format(task_prefix),
        "dro": "{}_dro_tuning".format(task_prefix),
    }
    ...
    ## ERM Group Aware
    result_df_group_aware, config_df_group_aware = load_dataframes(
        experiment_names["erm_group_aware"]
    )

    best_mean_auc_min = select_model_mean_min_max(
        result_df_group_aware,
        config_df_group_aware,
        metric_name="auc",
        agg_func_inner="min",
        agg_func_outer="max",
    )
    ...
    best_mean_auc_min = best_mean_auc_min.assign(
        config_selection="auc_min_max", tag="aware_auc_min"
    )
    ...
    best_mean_loss_max = best_mean_loss_max.assign(
        config_selection="loss_max_min", tag="aware_loss_max"
    )
    ...
    best_mean_loss_mean_balanced = best_mean_loss_mean_balanced.assign(
        config_selection="loss_mean_min_balanced", tag="aware_balanced"
    )
    # display(best_mean_loss_mean_balanced)
    selected_models_aware = (
        pd.concat([best_mean_auc_min, best_mean_loss_max, best_mean_loss_mean_balanced])
        .drop(columns="performance")
        .merge(config_df_group_aware)
        .assign(experiment_name=experiment_names["erm_group_aware"])
    )
    selected_models_aware.head()
    ...
    selected_models_dro = (
        pd.concat(
            [
                best_mean_auc_min_dro,
                best_mean_loss_max_dro,
                best_mean_auc_min_dro_by_objective_metric,
                best_mean_loss_max_dro_by_objective_metric,
            ]
        )
