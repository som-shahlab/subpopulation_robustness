...
def extract_time_series_from_subject(t_path):
    ...
    for i, stay_dir in enumerate(os.listdir(t_path)):
        ...
        try:
            ...
            continue
    print(
        "Number of patients with less than 15 or more than 200 records:", filter_15_200
    )
...
def convert_events_to_timeseries(events, variable_column="itemname", variables=[]):
    metadata = (
        events[["itemoffset", "patientunitstayid"]]
        .sort_values(by=["itemoffset", "patientunitstayid"])
        .drop_duplicates(keep="first")
        .set_index("itemoffset")
    )

    timeseries = (
        events[["itemoffset", variable_column, "itemvalue"]]
        .sort_values(by=["itemoffset", variable_column, "itemvalue"], axis=0)
        .drop_duplicates(subset=["itemoffset", variable_column], keep="last")
    )
    timeseries = (
        timeseries.pivot(
            index="itemoffset", columns=variable_column, values="itemvalue"
        )
...
def prepare_categorical_variables(root_dir):
    ...
    all_df = all_df[
        all_df.hospitaldischargestatus != 2
    ...
    all_df["Verbal"] = (
        all_df["Verbal"] + dxmax + etmax + gemax + totmax + eyemax + motmax
    )
    return all_df
...
def label_decompensation(all_dec):
    ...
    all_dec["count_max"] = all_dec.groupby(["patientunitstayid"])["temp_y"].transform(
        max
    )
    all_dec["label_24"] = np.nan
    all_dec.loc[all_dec["itemoffset"] < all_dec["count_max"], "label_24"] = 0
    all_dec.loc[all_dec["itemoffset"] >= all_dec["count_max"], "label_24"] = all_dec[
        "unitdischargestatus"
    ]
...
def diag_labels(diag):
    ...
    diag.loc[
        diag["icd"].isin(codes["renal_failure"]), "Acute and unspecified renal failure"
    ] = 1

    diag.loc[
        diag["icd"].isin(codes["Gastroint_hemorrhage"]), "Gastrointestinal hem"
    ] = 1
    ...
    diag.loc[
        diag["icd"].isin(codes["Acute_cerebrovascular"]),
        "Acute cerebrovascular disease",
    ] = 1
    ...
    diag.loc[
        diag["icd"].isin(codes["myocar_infarction"]), "Acute myocardial infarction"
    ] = 1
    diag.loc[
        diag["icd"].isin(codes["hypercomp"]), "Hypertension with complications"
    ] = 1
