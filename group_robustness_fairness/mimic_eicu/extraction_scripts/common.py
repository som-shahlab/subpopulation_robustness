...
def split(df, eval_weight=1, target_name="target"):
    ...
    skf = StratifiedKFold(
        n_splits=n_splits_train + eval_weight, random_state=42, shuffle=True
    )
    for c, (_, fold_index) in enumerate(
        skf.split(idx_train, df.loc[idx_train, target_name])
    ):
        df.loc[idx_train[fold_index], "fold_id"] = str(c)
    df.loc[
        df.fold_id.isin(
            list(map(str, range(n_splits_train, n_splits_train + eval_weight, 1)))
        ),
        "fold_id",
    ] = "eval"
    return df
...
def bin_age(age):
    ...
    for c, i in enumerate(bins):
        if (age >= i[0] and age < i[1]) or (c == len(bins) - 1 and age == i[1]):
            return f"{i[0]}-{i[1]}"


# from https://github.com/YerevaNN/mimic3-benchmarks/blob/master/mimic3models/preprocessing.py
class Discretizer:
    def __init__(
        self,
        timestep=0.8,
